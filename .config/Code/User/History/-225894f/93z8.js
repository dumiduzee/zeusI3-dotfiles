...
export const login = async () => {
  try {
    ...
    const response = await axios.post(
      `${API_BASE_URL}/login`,
      { username: 'dumi', password: 'dumi' },
      {
        headers: { 'Accept': 'application/json' },
        withCredentials: true, // Ensure cookies are stored for session management
      }
    );
...
export const getClientData = async (clientName) => {
  try {
    ...
    const response = await axios.post(
      `${API_BASE_URL}/panel/api/inbounds/clientIps/${clientName}`,
      null, // No request body needed for this GET request
      {
        headers: { 'Accept': 'application/json' },
